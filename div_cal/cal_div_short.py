...
def get_div_by_year():
    ...
    OUTPUT_MERGE_TABLE = pd.merge(MV_TABLE, DIV_YEAR_TABLE, how='left', on=['stockcode', 'report_year'])
    # 保存合并后的 代码-日期-分红表
    OUTPUT_MERGE_TABLE.rename(columns={i: i + '_l0' for i in RENAME_COLUMN}, inplace=True)  # 重命名列便于区分
    OUTPUT_MERGE_TABLE.to_parquet('merge.parquet', index=False)
...
def get_exp_div():
    ...
    # ----------------保存结果----------------#
    # 输出真实值线性外推预测值
    # DIV_L0是T-1期年报真实值,0表示在当前交易日T没有信息
    # DIV_L01是计算的静态分红,在DIV_L0为0时,使用了T-2年年报的分红
    # PRED_COLUMN是在当前交易日T使用参数 LAG_PERIOD 根据历史滞后分红计算出的OLS线性预测值
